describe 'message source snapshots', :golf do
  context 'with a message source that has no snapshot capability' do
    let(:history) do
      ...
      commit2 = Euston::Commit.new message_source_id: commit1.message_source_id, sequence: 2, events: [
        namespace::TeeBooked.v(1).new(course_id: course_id, player_id: player_id, time: time + 1000).to_hash
      ]

      Euston::MessageSourceHistory.new id: course_id, commits: [commit1, commit2]
    end
